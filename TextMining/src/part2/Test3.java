...
public class Test3 {
	 ...
	 * Sizes of labels
	 */
	public static int numOfAllCUIs = 2;
	public static int numOfAbstractLabels = 2;
	 ...
	 * IO
	 */
	public static String inputFile = "synthetic6";
	...
	public static void main(String[] args) throws Exception {
		...
		example.labels.add(0);
		example.labels.add(1);
		example.labels.add(0);
		example.labels.add(1);
		example.labels.add(1);

		// wordIndices
		example.wordIndices.add(0);
		example.wordIndices.add(1);
		example.wordIndices.add(1);
		example.wordIndices.add(1);
		example.wordIndices.add(0);
		example.wordIndices.add(0);
		...
		// prediction
		example.predictedRelation = 0;

		// observation
		example.observedRelation = 0;

		// distinct wordIndices
		example.distinctWordIndices.add(0);
		example.distinctWordIndices.add(1);

		// chain length
		example.chainLength = 6;

		TrainingExample example2 = new TrainingExample();

		// labels
		example2.labels.add(0);
		example2.labels.add(0);
		example2.labels.add(0);
		example2.labels.add(1);
		example2.labels.add(1);
		example2.labels.add(1);

		// wordIndices
		example2.wordIndices.add(1);
		example2.wordIndices.add(1);
		example2.wordIndices.add(1);
		example2.wordIndices.add(1);
		example2.wordIndices.add(1);
...
//		example2.distinctWordIndices.add(0);
		...
		TrainingExampleCollection collection = new TrainingExampleCollection();
		collection.examples = examples;
		collections.add(collection);
		double old_logLikelihood = 0;
		double new_logLikelihood;

		while (true) {
			calculateGradientsWithExactDenominator(collections);
			updateWeightMatricesFromGradientMatrices();
			new_logLikelihood = getLogLikelihoodWithExactDenominator(collections);
			System.out.println(new_logLikelihood);
			System.out
					.println("percentage: "
							+ getChangePercentage(old_logLikelihood,
									new_logLikelihood));
	...
	private static double computePartitionFunctionDerivativeEquivalentExponent(
		...
		double[] chainPredEquivalentExponents2 = new double[2 * numOfObservedValues];
		int wordIndex;
		...
		for (int pred = 0; pred < numOfPredictedValues; pred++) {
			...
			if (flag.equals("po") && index1 == pred
				...
				chainPredEquivalentExponents2[pred * 2 + 1] = predictedNodeObservedNodeMatrix[pred][example.observedRelation];
			else
	...
	private static void calculateGradientsWithExactDenominator(
		...
		for (int i = 0; i < collections.size(); i++) {
			...
			System.out.println("Calculating gradients, collection #" + i
					+ ", ending time is: " + System.currentTimeMillis() / 1000
					+ "s");
		}
	...
	public static void main2(String[] args) throws Exception {
		...
		// 834 666 150 292(index1_content) 152 220 480 278 379 307 397 548 546
		// 779 691 76 361 80
		...
		predictedNodeObservedNodeMatrix[0][0] = 3;
		predictedNodeObservedNodeMatrix[0][1] = 1;
		predictedNodeObservedNodeMatrix[1][0] = 1;
		predictedNodeObservedNodeMatrix[1][1] = 3;
		...
		observationDistribution = new EnumeratedIntegerDistribution(singletons,
				probabilities);
		...
		while (true) {
			...
			pause();
			// break condition 1
			// if (Math.abs((curExpectationLikelihood -
			// prevExpectationLikelihood)
			// / prevExpectationLikelihood) < expectationTerminalPercentage) {
